...
const HODDashboard = () => {
    ...
    const handleApprove = async (requestId) => {
        ...
        try {
            ...
            let nextStep = 'Principal';
            if (request?.targetDepartment && request?.currentLevel === 2) {
                nextStep = `${request.targetDepartment} HOD`;
            }

            setMessage({ type: 'success', text: `Request approved and forwarded to ${nextStep} for final review!` });
            fetchPendingRequests();
    ...
    const getRequestLevel = (request) => {
        if (request.currentLevel === 2) return 'Student\'s Department HOD';
        if (request.currentLevel === 3) return 'Target Department HOD';
        return `Level ${request.currentLevel}`;
    };
                        ...
                        {pendingRequests.map((request) => (
                            <div key={request._id} className="request-card">
                                <div className="request-header">
                                    ...
                                    <span className="badge pending">{getRequestLevel(request)}</span>
                                </div>

                                <div className="request-details">
                                    <p><strong>Student:</strong> {request.studentName}</p>
                                    <p><strong>Student's Dept:</strong> {request.studentDepartment}</p>
                ...
                <div className="modal-overlay" onClick={() => setSelectedRequest(null)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            ...
                            {selectedRequest.teacherApprovedByName && (
                                <div className="detail-section">
                                    ...
                                    <p><strong>Approved by:</strong> {selectedRequest.teacherApprovedByName}</p>
                                    <p><strong>Date:</strong> {new Date(selectedRequest.teacherApprovedDate).toLocaleDateString()}</p>
                                    {selectedRequest.teacherSignature && (
                                        <img src={selectedRequest.teacherSignature} alt="Teacher Signature" className="signature-img" />
                                    )}
                                </div>
                            )}
                        ...
                        <div className="modal-footer">
                            ...
                            >
                                {loading ? 'Processing...' :
                                    selectedRequest.targetDepartment && selectedRequest.currentLevel === 2 ?
                                        'Approve & Forward to Target HOD' :
